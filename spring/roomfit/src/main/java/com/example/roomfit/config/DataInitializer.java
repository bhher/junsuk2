...
public class DataInitializer {
	...
	private void seedMembers() {
		if (findSeedMember("admin", "admin@roomfit.local").isEmpty()) {
		...
		Member user1 = ensureSeedMember("user1", "user1@roomfit.local", () -> Member.builder()
				.loginId("user1")
		...
		Member user2 = ensureSeedMember("user2", "user2@roomfit.local", () -> Member.builder()
				.loginId("user2")
				.password(passwordEncoder.encode("user1234"))
				.name("이인테")
				.nickname("북유럽감성")
	...
	private void seedInteriorPosts() {
		...
		Member user1 = findSeedMember("user1", "user1@roomfit.local").orElse(null);
		Member user2 = findSeedMember("user2", "user2@roomfit.local").orElse(null);
	...
	private Optional<Member> findSeedMember(String loginId, String email) {
		return memberRepository.findByLoginId(loginId)
				.or(() -> memberRepository.findByEmail(email));
	}

	private Member ensureSeedMember(String loginId, String email, Supplier<Member> factory,
	                                String logMessage) {
		return findSeedMember(loginId, email).orElseGet(() -> {
			Member saved = memberRepository.save(factory.get());
			log.info(logMessage);
			return saved;
		});
	}
