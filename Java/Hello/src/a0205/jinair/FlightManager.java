...
public class FlightManager {
    ...
    //항공기 목록이 출력 -> flight 에서 tostring 이용해서
     ...
     public void bookFlight() throws InterruptedException { //예외를 호출한 쪽으로 던진다.
        //국제선 - 사용자 이름, 생년입력 받아서 나이확인하고 만 15 미만은 거절 
        for(;;){ //무한루프
            displayFlightList("항고편예매");
            System.out.print("예매할 항공편 입력 >");
            try {
                int bookNum = Integer.parseInt(sc.next());
                if(bookNum > flights.size() || bookNum < 1){
                    //예약할 수 잇는 목록의 갯수가 크거나 목록이 없으면
                    System.out.println("잘못된입력입니다.");
                    continue;
                    //유효하지 않은 입력을 받은경우 루프 다음 반복
                    //사용자에게 올바른 이력을 다시 요청
                }
        ...
        while(true){
            try {
                ...
                if(seatInt+ 1 < 1 || seatInt+1 >20){//좌석 1~20
                    ...
                    //좌석이 비어잇으면
                    flight.getSeats().set(seatInt, "XX"); //좌석수정 [ 1] -> [XX]
     ...
     private void passengerInfo(Flight flight) {
       ...
       while(true){
        try {
            ...
            if(!p.man15(p) && flight.getInternationalFlight()){//15세미만 국제선이면
